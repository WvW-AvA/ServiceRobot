...
struct Platform::TChannel
{
    ...
    TChannel(asio::io_service &_ioservice) : socket(_ioservice)
    {
    }
};
...
Platform::TGrader::TGrader() : grader(0),
...
void Platform::RunTest(unsigned int _test_idx)
{
    ...
    if (mGrader)
        mGrader->grader->BeginEvaluation(
    ...
    if (mLog)
        LogResult(_test_idx, es, ts, mResult.str());
}
...
void Platform::LogConfInfo()
{
    ...
    if (log)
    {
        ...
        if (mGrader.get())
        {
            log << "Grader path: " << mGrader->path << endl
                << endl;
        }
    }
    else
    ...
    while (true)
    {
        try
        {
            ...
            else if (action_name->get() == "Sense")
            {
                ...
                if (mGrader)
                    mGrader->grader->EvaluateSense(A);
...
boost::shared_ptr<Platform::TestDesc> Platform::getTestDesc(const std::string &file_path)
{
    ...
    do
    {
        ...
        if (att_mis && strcmp(att_mis, "on") == 0)
            desc->is_missing = true;

        const char *att_err = env->Attribute("err");
        if (att_err && strcmp(att_err, "on") == 0)
            desc->is_erroneous = true;

        const char *att_ans = env->Attribute("ans");
        if (att_ans && strcmp(att_ans, "on") == 0)
            desc->is_misleading = true;
